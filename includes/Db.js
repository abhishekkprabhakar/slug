function readPlace(data,callback){
	...
	var stateCode_o= getState(origin.state)
	var stateCode_d= getState(destination.state)
	if (callback)
		callback({ origin: { stateCode: stateCode_o }, destination: { stateCode: stateCode_d } });
